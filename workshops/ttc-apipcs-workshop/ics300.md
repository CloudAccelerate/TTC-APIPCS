...
# Lab 300 - API Platfrom Cloud Service
...
**1.2.2**	On the APIs page, click your API to open its details page.

**1.2.3**	Click the Implementation tab.



**1.2.4**	In the Available Policies section, expand Traffic Management, hover over Application Rate Limiting, and then click Apply.
...
5.	On the Apply Policy Dialog, complete these fields:
a.	(Optional) Complete the name and description fields as you would like.
b.	Ensure that Key Validation is selected from the Place Policy After list.
c.	Click Next.
d.	Enter a number between 1 and 10 into the Rate Limit Per Application field. This is the number of requests that will be passed during each time interval.
e.	Select Minute from the Time Interval list.
